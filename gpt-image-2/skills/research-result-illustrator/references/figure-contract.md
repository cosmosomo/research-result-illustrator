...
Statistics needed:
Source data needed:
Image-integrity notes, when relevant:
Trusted panel provenance and SHA-256, for exact-evidence mode:
Reserved chart boxes, for exact-evidence mode:
AI-generated conceptual elements:
Reviewer risk:
```
...
- Separate primary evidence from supporting evidence. The primary evidence gets
  ...
  visually quieter.
- If the user provides data but no claim, infer a provisional claim from the request
  and label it as provisional. Do not block an exploratory draft waiting for confirmation.
...
- If exact-evidence mode is requested, can every final data pixel be traced to a trusted source panel rather than GPT Image output?
- Are generated mechanisms and arrows clearly conceptual and scientifically reviewed?
