...
## GPT Image composite checks

For exact-evidence mode, run `scripts/research_illustrator.py verify --manifest <path>` before delivery. It reports mismatches as visible warnings. Add `--strict` only for a manuscript, archive, or user request where a mismatch must block delivery. Then inspect the composite and confirm:

- no generated number, axis, p-value, error bar, scale bar, legend, or microscopy region is presented as measured evidence;
- reserved chart boxes contain the original deterministic panels without crop, blend, occlusion, or stretch;
- generated entities, causal arrows, apparatus, anatomy, and labels are scientifically defensible;
- source charts, AI schematic, final PNG, prompt, provenance JSON, and non-secret configuration summary are bundled together.
